...
- Q: Session tracking and user identity strategy? → A: Session-per-browser-tab (UUID in sessionStorage, expires on tab close, new session per tab)
- Q: Chunk overlap strategy for text processing? → A: 20% overlap (100-160 tokens overlap for 500-800 token chunks to preserve context at boundaries)
- Q: Citation link format to textbook sections? → A: Section anchors with hash (e.g., `/docs/page-name#heading-id` using Docusaurus auto-generated heading anchors)
...
- **FR-008**: System MUST generate and store unique session UUID in browser sessionStorage on first chat interaction

#### RAG Pipeline & Agent Orchestration
- **FR-009**: Retrieval Agent queries Qdrant for similar chunks
- **FR-010**: Returns top-k chunks above threshold (default: k=5, threshold=0.7)
- **FR-011**: Answer Agent synthesizes response from context
- **FR-012**: No information beyond retrieved context (no hallucination)
- **FR-013**: Citation Agent identifies source chunks
- **FR-014**: Citations formatted as clickable links using Docusaurus heading anchors (format: `/docs/page-name#heading-id`)
- **FR-015**: Selection-based prioritization of context
- **FR-016**: Guardrails for out-of-scope queries
- **FR-017**: System MUST detect non-English queries and respond with English-only scope message

#### Context7 MCP Server Integration
- **FR-018**: Uses MCP fs tools to read markdown files
- **FR-019**: Chunks content with 500-800 token target and 20% overlap between adjacent chunks
- **FR-020**: Extracts metadata including file_path, title, heading hierarchy, and Docusaurus heading anchor IDs
- **FR-021**: MCP server accessible via async calls
- **FR-022**: Uses MCP system tools for sync scheduling

#### Embeddings & Vector Storage
- **FR-024**: OpenAI Embeddings API (text-embedding-3-large)
- **FR-025**: Stores in Qdrant Cloud with metadata
- **FR-026**: Qdrant collection with matching dimensions
- **FR-027**: Upsert logic to avoid duplicates
- **FR-028**: Deletes embeddings for removed documents

#### Backend API (FastAPI)
- **FR-029**:  endpoint for questions
- **FR-030**:  endpoint for contextual queries
- **FR-031**:  endpoint for manual embedding
- **FR-032**: JSON responses with answer, citations, sources
- **FR-033**: Async request handling
- **FR-034**: Input validation and error responses
- **FR-035**: CORS for Docusaurus domain

#### Database & Logging (Neon Postgres)
- **FR-036**:  table for queries
- **FR-037**:  table for results
- **FR-038**:  table for ratings
- **FR-039**: Structured logging of operations
- **FR-040**: Analytics query interface

#### Content Synchronization
- **FR-041**: Detects markdown file changes
- **FR-042**: Triggers re-embedding for modifications
- **FR-043**: Configurable sync schedule (default: 6 hours)
- **FR-044**: Graceful error handling and continuation
- **FR-045**: Tracks sync state in database

### Key Entities

- **TextbookChunk**: chunk_id, content_text, file_path, document_title, heading_hierarchy, section_anchor (heading ID), chunk_index, overlap_tokens (100-160), embedding_vector, created_at, updated_at
- **Query**: query_id, query_text, user_session_id (UUID from sessionStorage), retrieved_chunk_ids, answer_text, citations, similarity_scores, timestamp
...
9. Session tracking uses browser sessionStorage with auto-generated UUID per tab (no cookies, expires on tab close)
